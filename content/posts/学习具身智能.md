...
流匹配vla模型的强化学习原理。由于有去噪步骤，无法直接计算对数概率
非流匹配，比如openvla-oft模型的强化学习原理，这类模型输出动作通过MLP，可以直接计算动作的对数概率

## VLA 动作大模型 复现+手写

以qwen2.5vl-3b为基础，实现oft动作头、flow-match动作头，训练libero即可
在isaaclab中训练so101机械臂
修改so101机械臂，尝试设计自己的
...
## 【论文 2025.9 *****】simpleVLA-RL

改进verl框架，使用grpo训练openvla-oft模型，grpo中移出了KL散度项，增加探索性
仿真环境：libero、roboTwin2.0
流程：比如一个libero任务，先用50条微调openvla-oft，然后用微调的模型在这个环境上做grpo强化训练，效果会有明显提升
值得复现
https://github.com/PRIME-RL/SimpleVLA-RL  


## 【博客 *****】在机器人领域应用深度强化学习，目前主流的一些思路是什么？ - 飞鸟出林的回答 - 知乎
https://www.zhihu.com/question/526622947/answer/2482611442

详细列了四足的最新发展成功，控制问题基本被ETH的强化学习解决了，现在ETH关注在模仿学习，因为强化学习学习到的动作不可控，模仿学习比如AMP更好
